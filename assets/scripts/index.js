...
form.addEventListener("submit", async function (e) {
  ...
  const handle = form.elements.query.value; // Getting the value of the input field
  try {
    ...
    full.classList.remove("d-none"); // Removing the d-none class from the div with id vis

    // Getting the required data from the response in the form of JSON
    ...
    // Setting the values of the required elements
    ...
    document.getElementById("contribution").innerText =
    ...
    for (var i = 0; i < userStatus.data.result.length; i++) {
      ...
      var unixTime = sub.creationTimeSeconds; // Unix time
      var ms = unixTime * 1000; // Convert to milliseconds
      var d1 = new Date(ms); // Convert to date object
      var d2 = new Date(d1.getFullYear(), d1.getMonth(), d1.getDate()); // Convert to date object
      var val = d2.getTime() / 1000; // Convert to unix time
    ...
    function drawChart() {
      ...
      var chart6 = new google.visualization.BarChart(
      ...
      var chart7 = new google.visualization.BarChart(
        document.getElementById("chart_7")
      );
